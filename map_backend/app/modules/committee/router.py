...
@router.get(
    ...
    summary="查询会议列表",
    description="按创建时间倒序返回所有会议，供会议中心展示。",
)
async def list_meetings(
    db: DBSession,
    _user_id: CurrentUserID,
    status: str | None = Query(None, description="按状态过滤：DRAFT/VOTING/PUBLISHED"),
) -> list[MeetingResponse]:
    meetings = await services.list_meetings(db, status=status)
