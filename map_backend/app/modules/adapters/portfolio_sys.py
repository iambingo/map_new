...
class MockPortfolioAdapter:
    ...
    async def trigger_rebalance(self, payload: dict) -> dict:
        """Mock 调仓：记录日志并返回模拟结果。"""
        logger.info("MockPortfolioAdapter: simulated rebalance for %s", payload)
        return {"status": "rebalance_requested", "portfolio_id": payload.get("portfolio_id")}

    async def fetch_snapshot(self, payload: dict) -> dict:
        """Mock 快照：返回当前组合头寸的简化字典。"""
        positions_data = await self.get_all_positions(payload.get("portfolio_id", "MAP-MAIN-001"))
        return positions_data.model_dump(mode="json")
...
class PortfolioSystemAdapter(BaseAdapter):
    ...
    def __init__(self) -> None:
        self.base_url = settings.PORTFOLIO_SYS_URL
        super().__init__()
    ...
    async def trigger_rebalance(self, payload: dict) -> dict:
        """触发真实组合系统调仓。"""
        raw = await self._post("/portfolios/rebalance", json=payload)
        return raw

    async def fetch_snapshot(self, payload: dict) -> dict:
        """获取真实组合系统快照。"""
        portfolio_id = payload.get("portfolio_id", "MAP-MAIN-001")
        raw = await self._get(f"/portfolios/{portfolio_id}/snapshot")
        return raw
...
def get_portfolio_adapter() -> MockPortfolioAdapter | PortfolioSystemAdapter:
    ...
    """
    portfolio_url = settings.PORTFOLIO_SYS_URL
    if portfolio_url:
...
# ── 模块级单例（供 consumers/handlers.py 和 orchestrator/commands.py 直接导入）

portfolio_adapter: MockPortfolioAdapter | PortfolioSystemAdapter = get_portfolio_adapter()
